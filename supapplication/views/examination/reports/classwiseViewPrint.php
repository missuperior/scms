...
<!--    <th width="3%" height="82" align="center" valign="top" bgcolor="#FFFFFF" scope="col">
  ...
  <?php 
        ...
        for($c=0; $c < count($students); $c++){ 
            $fail_subjects  =   $this->Examination_model->getFailSubjects($students[$c]['student_id'],$semester);
...
<tbody>
  <tr>
    ...
    <td align="center" valign="top" style="font-size: 11px">
        <?php 
            if($exam_type == 'Mid'){ 
                ...
                $res    = $this->Examination_model->calculateGpa($students[$i]['obtained'],$students[$i]['credit_hours']);
                $gpa    =   $gpa + $res; 
            }else{ 
    ...
    <?php } }
            ...
            $gpaa = $gpa/$total_crdt_hours;
            if($students[$c]['semester'] > 1){
                $cgpa   =   $this->Examination_model->getCGPA($student_id,$semester);           
                //$cgpa   =   $this->Examination_model->getLastGpa($students[$c]['student_id'],$gpa,$total_crdt_hours);           
            }else{
                $cgpa = $gpaa;
            }
