            ...
            <table style="border-style: solid; border-width: 1px; width:100%; float:left;"  class="table">
                <tbody style="align:center; width:100%;">
                    ...
                    foreach ($result AS $key => $res) {
                        ...
                        $mresult = $this->Examination_model->mid_result($student_id, $row['session_id'], $res['course_id'],$batch_id);
                        $fresult = $this->Examination_model->final_result($student_id, $row['session_id'], $res['course_id'],$batch_id);

//                        print_r($fresult);
//                        echo '<br>';
                        //echo $student_id .'==='. $batch_id .'==='.  $res['course_id'] .'==='. $row['session_id'];
                        ...
                        if ($labi != null) {
                            ...
                            //echo   'Lab CR'.$cr_hrs.'=='. $res['course_id'].'<br/>' ;
                        } else {
                            $total = $mresult[0]['midd'] + $fresult[0]['final'];
                            $gpa = $this->Examination_model->getGpa($total, $cr_hrs);
                            //echo   'NO Lab CR'.$cr_hrs.'=='.$res['course_id'].'<br/>' ;
                        }

                        //$gpa = $this->Examination_model->getGpa($total, $fresult[0]['credit_hours']);
                        $cgpaa = $cgpaa + $gpa;
                        $total_subjects ++;
...
<!--                            <td style="border-style: solid; border-width: 1px; width:12%; "><p style="text-align: center;"><?php echo $fresult[0]['credit_hours']; ?></p></td>-->
                            <td style="border-style: solid; border-width: 1px; width:12%; "><p style="text-align: center;"><?php echo $cr_hrs; ?></p></td>
