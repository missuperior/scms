...
<table style="border-style: solid; border-width: 1px; width:100%; float:left;"  class="table">
    ...
    <?php   
            ...
            foreach ($result AS $key => $res){
                    ...
                    $mresult    = $this->Examination_model->mid_result($student_id,$row['session_id'],$res['course_id']);
                    $fresult    = $this->Examination_model->final_result($student_id,$row['session_id'],$res['course_id']);

                    //print_r($fresult);die;
                    
                    // hceck lab
                    $labi       =  $this->Examination_model->getLabMarks($student_id,$batch_id,$res['course_id'], $row['session_id']);
                    ...
                    $gpa        = $this->Examination_model->getGpa($total);
                    $cgpaa       = $cgpaa + $gpa;
                    $total_subjects ++;
                    ...
                    {   $grade = 'C';}
                    elseif($total >= 70 && $totall < 80)
                    {   $grade = 'B';}
                    elseif($total >= 80 && $totall <= 100)
                    {   $grade = 'A';}
...
<tbody style="align:center; width:100%;">
		...
		<th style=" width:20%; "><p style="text-align: center; font-size:12px; ">GPA : <?php echo number_format($cgpaa/$total_subjects, 2, '.', ''); ?></p></th>

		</tbody>
</table>

        <?php 
            $cgpa                   =   $cgpa + number_format($cgpaa/$total_subjects, 2, '.', '');
...
<table style=" width:100%; float:left; margin-top: 20px;" class="table" >   
    <tbody style="align:center; width:100%;  font-size: 11px; font-weight: normal;">
                    ...
                    <th style="width:15%; "><p style="text-align: left; "><b>CGPA : </b><?php echo ' '.$cgpa/$c; ?></p></th>                                        
    </tbody>
</table>
