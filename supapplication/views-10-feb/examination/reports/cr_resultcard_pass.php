...
<table style="border-style: solid; border-width: 1px; width:100%; float:left;"  class="table">
    ...
    <?php   
            ...
            foreach ($result AS $key => $res){
                    ...
                    $mresult    = $this->Examination_model->mid_result_pass($student_id,$res['course_code'],$batch_id);
                   
                    $fresult    = $this->Examination_model->final_result_pass($student_id,$res['course_code'],$batch_id);

                   // echo '<pre>';print_r($fresult);die;
                    
                    // hceck lab
                    $labi       =  $this->Examination_model->getLabMarks_pass($student_id,$batch_id,$fresult[0]['course_id']);
                                        
                    $cr_hrs     = $fresult[0]['credit_hours'];
                    
                    if($labi != null){
                        $total      = $mresult['midd']+$fresult[0]['final']+$labi[0]['final_value_1'];
                        $cr_hrs = $cr_hrs+1;
                        $gpa    = $this->Examination_model->getGpa($total, $cr_hrs );
                    }else{
                        $total      = $mresult['midd']+$fresult[0]['final'];
                        $gpa        = $this->Examination_model->getGpa($total, $cr_hrs);
                    }
        ...
        ?>
        <?php 
                $duplicate      =   $this->Examination_model->findCourseInPreviousSession($student_id,$row['session_id'],$res['course_code']);
                if($duplicate == 0){?>
        <tr>
