...
//        echo '<pre>';print_r($result['students']);die;
    ...
    public function add_final_result_form($campaign_id, $program_id, $course_id, $semester) {
        ...
        if (count($result['mid_res']) > 0) {
            $res = $this->Examination_model->getFinalStructure(array('program_id' => $program_id, 'course_id' => $course_id, 'campaign_id' => $campaign_id));
    ...
    public function calculate_save_std_gpa($campaign_id, $program_id, $semester) {
        ...
        foreach ($students AS $row) {
            ...
            for ($c = 0; $c < count($final_marks); $c++) {
                ...
                $res = $this->Examination_model->getGpa($marks, $final_marks[$c]['credit_hours']);
                $gpa = $gpa + $res;
            }
            ...
            if (count($check) > 0) {
                $std_gpa_id = $this->Examination_model->UpdateStdgpa($row['student_id'], $gpa, $semester, $total_gpa, $total_credit_hours);
            } else {
                $std_gpa_id = $this->Examination_model->SaveStdgpa($row['student_id'], $gpa, $semester, $total_gpa, $total_credit_hours);
            }
        }
    }
...
//        $result['students']     =   $this->Examination_model->getStudent($campaign_id,$program_id);  
    ...
    public function view_mid_result($campaign_id, $program_id, $course_id, $semester) {
        ...
        $res_status = $this->Examination_model->getMidResStatus(array('program_id' => $program_id, 'course_id' => $course_id, 'campaign_id' => $campaign_id, 'semester' => $semester));
...
//      echo '<pre>';print_r($result['students']);die;
    ...
    // >>>>>>>>>>>>>>>>>>>>>   Topper List For Course Registration Session Wise    <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  //


    public function topperList_cr() {
...
//        echo '<pre>'; print_r($students);die;
       ...
       //  echo '<pre>'; print_r($array);die;
        ...
        $this->load->view('admin_ace/admin_header');
        $this->load->view('examination/cr/TopperView', $result);
        $this->load->view('admin_ace/admin_footer');
    }
...
//        echo '<pre>';print_r($result);die;
    ...
    public function cr_student_result_pass() {
        ...
        $result['sessions'] = $this->Examination_model->getSessionsWithOutSummer($result['student_info']->student_id);
...
//        echo '<pre>';print_r($res);
    ...
    public function update_mid_res_cr() {
        ...
        if ($update > 0) {
            ...
            $this->calculate_save_std_gpa_std_cr($session_id, $student_id);
...
//        die;
        ...
        $res_status = $this->Teachers_model->getMidResStatus($program_id, $course_id, $section, $batch_id, $session_id);
        $result['res_status'] = $res_status->result_status;

        $res = $this->Teachers_model->getMidStructure($program_id, $course_id, $section, $batch_id, $session_id);
...
//        echo '</pre>';die;
        ...
        $res_status = $this->Teachers_model->getFinalResStatus($program_id, $course_id, $section, $batch_id, $session_id);
        $result['res_status'] = $res_status->result_status;

        $res = $this->Teachers_model->getFinalStructure($program_id, $course_id, $section, $batch_id, $session_id);
    ...
    public function update_final_result_cr() {
        ...
        $this->calculate_save_std_gpa_cr($teacher_id, $program_id, $course_id, $course_section, $batch_id, $session_id, $course_type);
    ...
    public function delete_final_result_cr() {
        ...
        $this->calculate_save_std_gpa_cr_delete($program_id, $section, $batch_id, $session_id);
...
//                    echo '<pre>'; print_r($result['info']);die;
    ...
    Public function view_final_result_summary_cr() {
        ...
        $result['students'] = $this->Examination_model->classWiseFinalReport_cr_Jal($program_id, $section, $batch_id, $session_id);

        $result['offered'] = $this->Course_model->getofferedProgCoursesTheory($batch_id, $program_id, $session_id);
...
//      echo '<pre>';print_r($result['students']);echo '</pre>';
        ...
        $this->load->view('examination/cr/ClassWiseSummary_Jal', $result);
        $this->load->view('admin_ace/admin_footer');
    }
    ...
    Public function view_final_result_summary_cr_print() {
        ...
        // echo '<pre>';print_r($result['offered']);die;
        //$this->load->view('examination/cr/classwiseFinalViewPrint', $result);
        $this->load->view('examination/cr/ClassWiseSummaryPrint_Jal', $result);
    }

    // for updating the student session gpa

    public function calculate_save_std_gpa_cr($teacher_id, $program_id, $course_id, $course_section, $batch_id, $session_id, $course_type) {
    ...
    // for updating the student session gpa when final result deleted

    public function calculate_save_std_gpa_cr_delete($program_id, $course_section, $batch_id, $session_id) {

        $students = $this->Teachers2_model->getStudentsDelete($program_id, $course_section, $batch_id, $session_id);
...
//          echo '<pre>';print_r($final_marks);die;
        ...
        if (count($check) > 0) {
            $std_gpa_id = $this->Examination_model->UpdateStdgpa($student_id, $gpa, $semester, $total_gpa, $total_credit_hours);
        } else {
            $std_gpa_id = $this->Examination_model->SaveStdgpa($student_id, $gpa, $semester, $total_gpa, $total_credit_hours);
        }
    }

    // for single student

    public function calculate_save_std_gpa_std_cr($session_id, $student_id) {

        $final_marks = $this->Teachers2_model->getFinalMarks($student_id, $session_id);
        // echo '<pre>';print_r($final_marks);die;
        $gpa = 0;
        $total_credit_hours = 0;
        for ($c = 0; $c < count($final_marks); $c++) {
            $total_credit_hours = $total_credit_hours + $final_marks[$c]['credit_hours'];
            $marks = $final_marks[$c]['obtained1'] + $final_marks[$c]['obtained2'];
            $res = $this->Teachers2_model->calculateGpa($marks, $final_marks[$c]['credit_hours']);
            $gpa = $gpa + $res;
        }
        ...
        $check = $this->Teachers2_model->checkgpa($student_id, $session_id);
        if (count($check) > 0) {
            $std_gpa_id = $this->Teachers2_model->UpdateStdgpa($student_id, $gpa, $session_id, $total_gpa, $total_credit_hours);
        } else {
