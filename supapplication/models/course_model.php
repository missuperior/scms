...
//        return $query->result_array();
        ...
        $this->db->join('programs', 'courses.program_id = programs.program_id', 'inner');
        $this->db->join('batch', 'batch.batch_id = courses.batch_id', 'inner');
    ...
    function allAllocatedCourses()
    {
        ...
        $this->db->join('batch', 'courses_allocation.batch_id = batch.batch_id', 'inner');
...
//        $this->db->where( 'hr_employee_record.campus_id' , '3');
        $this->db->order_by( 'hr_employee_record.employee_name' , 'ASC');
...
/* added by zohaib*/
    ...
    function total_prg_students_list($session_id , $program_id){
        $query      = 
                "SELECT "
                . "students.student_id "
                . "FROM forms "
    ...
    function total_sections_students_list($session_id , $program_id){
        $query = "SELECT *  FROM student_sections WHERE session_id = $session_id and program_id = $program_id";
        $query_data = $this->db->query($query);
        return $query_data->result_array();
    }
    ...
    function getfirstSections($batch_id,$program_id, $session)
    {
        $query      = " SELECT program_section
                        ...
                        AND student_sections.program_id = '$program_id'
    ...
    function getofferedProgCoursesTheory($batch_id,$program_id,$session_id){
        $query      = " SELECT *
                        FROM `offered_courses`
                        INNER JOIN batch ON offered_courses.batch_id = batch.batch_id
...
//        echo $this->db->last_query();die;
    ...
    /* added by zohaib*/
    function allocatedCourseSectionsSettings($cur_session_id , $emp_id , $batch_id , $program_id){
          $query      = "
                        SELECT 
                        ...
                        WHERE course_sections_settings.session_id='$cur_session_id'
                        AND course_sections_settings.batch_id = $batch_id 
                        AND course_sections_settings.advisor_id = $emp_id 
                        AND course_sections_settings.program_id = $program_id ";
         
                        //WHERE prg_manager_id='$prg_mng_id' AND offered_courses.session_id='$cur_sessin_id'";
        $query_data = $this->db->query($query);
        return $val =  $query_data->num_rows() > 0 ? $query_data->result_array(): null;
    }
 
    
    function getcoursestd_list($section, $course_id, $session_id, $batch_id , $program_id){
        $query      = "
            SELECT * FROM `student_course_sections` 
            INNER JOIN students on students.student_id  =   `student_course_sections`.student_id
            ...
            AND student_course_sections.current_session_id  = $session_id AND student_course_sections.batch_id = $batch_id AND student_course_sections.program_id = $program_id";
        //die;
    ...
    function getTheoryCourses($program_id ,$bacth_id ){
    ...
    function alreadyCourseExists($course_array)
    {       
        $query = $this->db->get_where('courses', $course_array);
//        echo $this->db->last_query();
//        echo '<br/>';
        return $query->num_rows();
    ...
    function chkDuplicateAllocatedCourse($course_array)
    {       
        $query = $this->db->get_where('courses_allocation', $course_array);
        return $query->num_rows();
    }
} 