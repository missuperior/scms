     ...
     // **************START           Entry Test Module         ************** \\
        ...
        // check duplication of test name
            ...
            {                
                ...
                $query  = $this->db->query("SELECT count(*)AS students FROM initial_form where program_id = $program_id and form_no like '%S16%' and created_date >= '$start_date' ");
                $result = $query->result_array();
...
//                echo $this->db->last_query();die;
		...
		return $query->result_array();
            ...
            function getStartIdForm($programid){
                $campaign_code    =   $this->session->userdata('campaign_code');
                $s_d    =   '2016-01-09';
                $query      =   $this->db->query("SELECT * FROM initial_form WHERE program_id = $programid AND form_no like '%$campaign_code%'  ORDER BY initial_form_id ASC limit 1");
                $result     =   $query->row();
