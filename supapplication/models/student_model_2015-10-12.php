...
class Student_model extends CI_Model {
    ...
    function studentLogin($roll_no,$password)
    {
      ...
      $this->db->where('student_logins.roll_no',$roll_no);
      $this->db->where('student_logins.password',$password);
      $query = $this->db->get();   
    ...
    function getStudentProfile($student_id)
    {
      $this->db->select('students.*, 
                         forms.student_name, forms.father_name, forms.nic_no, forms.email, forms.mobile, forms.dob,forms.form_submit_date,
                         batch.*,
                         sessions.*,
