...
/* added by zohaib*/
    ...
    function single_section_students($session_id , $program_id, $section , $batch){
        $query = "SELECT *  FROM student_sections WHERE session_id = $session_id and program_id = $program_id and program_section = '$section' and batch = '$batch' ";
        $query_data = $this->db->query($query);
        return $val =  $query_data->num_rows() > 0 ? $query_data->num_rows() : null;
    }
    ...
    function allocatedCourseSectionsSettings($cur_session_id , $emp_id){
        $query      = "SELECT course_sections_settings.* ,courses.course_name  ,courses.course_id, courses.course_code, hr_employee_record.employee_name  , hr_employee_record.emp_id FROM course_sections_settings 
