...
//                  echo $this->db->last_query();die;
  ...
  // Bank cash report 
     ...
     function dateWiseBankCashReport($session_id,$shift,$campus_id, $campaign_id, $program_id,  $start_date, $end_date)
    { 
      
      $session     =    $session_id == 0 ? '' : "AND installments.session_id =". $session_id;
...
//      echo $this->db->last_query();die;       
    ...
    function programWiseBankCashReport($session_id,$shift,$campus_id, $campaign_id, $program_id, $type, $start_date, $end_date)
    { 
      ...
      if($campus_id == 0){
        ...
        else if($campus_id == 1 || $campus_id == 3)
        {
            ...
            $campus     =   "forms.campus_id = ".$campus_id;
                ...
                if($this->session->userdata('account_role_id') == 5){  $campus   =  "forms.campus_id = ".$campus_id ;   }
                 else{ $campus   =   "forms.campus_id = ".$this->session->userdata('campus_id') ; }
        }
...
//      echo $this->db->last_query();die;
    ...
    { 
        ...
        // for out station campus
         ...
         if($this->session->userdata('role') == 'OS' || ($campus_id != 1 && $campus_id != 3))
             {
                    $query = $this->db->query("SELECT installments.*, challan.*, students.student_id, students.roll_no, students.form_id, forms.campus_id,forms.gender, 
         ...
         if($this->session->userdata('role') == 'OS' || ($campus_id != 1 && $campus_id != 3))
             {
                    ...
                    $query = $this->db->query("SELECT installments.*, challan.*, students.student_id, students.roll_no, students.form_id, forms.campus_id, forms.student_name,forms.gender, forms.mobile, campaign.campaign_name, campus.campus_name, programs.program_name
                                      FROM forms_os AS forms
                                      INNER JOIN students_os AS students ON forms.form_id = students.form_id
                                      INNER JOIN installments_os AS installments ON students.student_id = installments.student_id
                                      INNER JOIN challan_os AS challan ON installments.installment_id = challan.installment_id
                  ...
                  $query = $this->db->query("SELECT installments.*, challan.*, students.student_id, students.roll_no, students.form_id, forms.campus_id,forms.gender, forms.student_name, forms.mobile, campaign.campaign_name, campus.campus_name, programs.program_name
                                      FROM forms
                                      INNER JOIN students ON forms.form_id = students.form_id
                                      INNER JOIN installments ON students.student_id = installments.student_id
                                      INNER JOIN challan ON installments.installment_id = challan.installment_id
...
//      echo $this->db->last_query();die;
   ...
   function getCampusClosinName($campus)
    ...
    function campusRevenueSummary($campaign_id, $campus_id,$status){
        ...
        if($status == 0){ $stats    =   '';}
        if($status == 1){ $stats    =   'AND students.status = "ok"';}
        if($status == 2){ $stats    =   'AND students.status != "ok"';}
        
         if($this->session->userdata('role') == 'OS' || ($campus_id != 1 && $campus_id != 3))
             {
                $query = $this->db->query("SELECT COUNT(students.`student_id`) AS total_students,
                                            SUM(srf.admission_fee)AS total_adm_fee,
                                            SUM(srf.session_fee) AS total_session_fee,
                                            SUM(srf.misc_fee) AS total_misc_fee,
                                            SUM(srf.`session_fee`+srf.`misc_fee`) AS total_wo_adm,
                                            SUM(srf.admission_fee+srf.`session_fee`+srf.`misc_fee`) AS total_w_adm,
                                            programs.`program_name`,campus.campus_name,campaign.campaign_name
                                            FROM
                                            students_os AS students
                                            INNER JOIN forms_os AS forms ON forms.`form_id` = students.`form_id`
                                            INNER JOIN student_fee_package_os AS srf ON students.`student_id` = srf.`student_id`
                                            INNER JOIN programs ON programs.`program_id` = srf.`program_id`
                                            INNER JOIN campus ON campus.`campus_id` = forms.`campus_id`
                                            INNER JOIN campaign ON campaign.`campaign_id` = forms.`campaign_id`
                                            WHERE
                 ...
                 $query = $this->db->query("SELECT COUNT(students.`student_id`) AS total_students,
                                            ...
                                            students
                                            INNER JOIN forms ON forms.`form_id` = students.`form_id`
                                            INNER JOIN student_fee_package AS srf ON students.`student_id` = srf.`student_id`
                                            INNER JOIN programs ON programs.`program_id` = srf.`program_id`
                                            INNER JOIN campus ON campus.`campus_id` = forms.`campus_id`
                                            INNER JOIN campaign ON campaign.`campaign_id` = forms.`campaign_id`
                                            WHERE
    ...
    // ****************** Fro Audit Reports StarT*********************** 
    
    function getAllIns($campaign_id,$campus_id,$program_id,$start_date,$end_date){
        ...
        if($start_date != '' && $end_date  != ''){
            $date = "AND sfp.`created_date` BETWEEN '$start_date' AND '$end_date'";
        }else{
            $date = '';
        }
         ...
         if($this->session->userdata('role') == 'OS' || ($campus_id != 1 && $campus_id != 3))
             {
                    ...
                    $query = $this->db->query("SELECT  
                                                students.student_id,students.enrolled_session_id,students.roll_no,forms.form_no,forms.student_name,programs.program_name,
                                                campaign.campaign_name, campus.campus_name,sfp.admission_fee,sfp.`misc_fee`,sfp.`session_fee`,students.status 
...
//                echo $this->db->last_query();die;
      ...
      function getAllInstallments($student_id,$session_id,$campus_id)
    {
      ...
      function getPaidInstallments($student_id,$session_id,$campus_id)
    {
        ...
        // for out station campus
         ...
         if($this->session->userdata('role') == 'OS' || ($campus_id != 1 && $campus_id != 3))
             {        
                 $query = $this->db->query("SELECT ins.*,challan.post_date
                                                FROM installments_os AS ins                                                 
                                                INNER JOIN challan_os AS challan ON ins.`installment_id` = challan.`installment_id`
                                                WHERE ins.`student_id` = $student_id AND ins.`session_id` = $session_id and challan.status = 1
                                            ");
                                                    
             }else{
                 $query = $this->db->query("SELECT ins.*,challan.post_date
                                                FROM installments AS ins                                                 
                                                INNER JOIN challan ON ins.`installment_id` = challan.`installment_id`
                                                WHERE ins.`student_id` = $student_id AND ins.`session_id` = $session_id and challan.status = 1
                                            ");
             }
    ...
    function getInsSum($student_id,$session_id,$campus_id){
        
        // for out station campus
         
         if($this->session->userdata('role') == 'OS' || ($campus_id != 1 && $campus_id != 3))
             {        
                 $query = $this->db->query("SELECT SUM(ins.payable) AS recable, SUM(IF(challan.status = 1, ins.payable,0))AS recvd
                                                FROM installments_os AS ins
                                                INNER JOIN challan_os AS challan ON challan.`installment_id` = ins.`installment_id`
                                                WHERE
                                                ins.`student_id` = $student_id AND ins.`session_id` = $session_id
                                            ");
