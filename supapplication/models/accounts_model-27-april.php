...
class Accounts_model extends CI_Model {
    ...
    { 
        ...
        if($this->session->userdata('role') == 'OS')
        {
            ...
            $query =  $this->db->query("SELECT count(*)AS total_students From forms_os AS form inner join students_os ON students_os.form_id = form.form_id Where students_os.roll_no = '' AND form.campus_id = $campus_id ");
        }else{
            $query =  $this->db->query("SELECT count(*)AS total_students From forms AS form inner join students ON students.form_id = form.form_id Where students.roll_no = '' ");
        }
    ...
    function prospectus()
    {
        ...
        {
            $this->db->where('campus_id',$this->session->userdata('campus_id'));
            return $this->db->count_all_results('prospectus_os');
        }else{
    ...
    function initial()
    {
        ...
        {
            $this->db->where('campus_id',$this->session->userdata('campus_id'));
            return $this->db->count_all_results('initial_form_os');
        }else{
    ...
    function detailed()
    {
        ...
        {
            $campus_id = $this->session->userdata('campus_id');
            $query =  $this->db->query('select count(*) from forms_os where inquiry_id != "0" AND campus_id = '.$campus_id.' ');
        }else{
    ...
    function student()
    {
        ...
        {
            $campus_id = $this->session->userdata('campus_id');
            $query =  $this->db->query('select count(*) from students_os INNER JOIN forms_os ON forms_os.form_id = students_os.form_id where inquiry_id != "0"  AND forms_os.campus_id = '.$campus_id.' ');
        }else{
...
// add installments
 ...
 function getStudentPackageInfo($student_id)
 {
     ...
     if($this->session->userdata('role') == 'OS')
     {
            $this->db->select('students.student_id,students.status,students.roll_no,students.current_session_id');
            $this->db->select('forms.program_id,forms.student_name,forms.campaign_id');
            $this->db->select('std_f_pkg.session_total_package,std_f_pkg.admission_fee,std_f_pkg.misc_fee,std_f_pkg.session_fee,std_f_pkg.remarks');
...
function getStudentInstallments2($student_id,$session_id)
 ...
 //  add roll no 
    ...
    function addRollNo($student_id,$roll_no,$operator_id)
    {
         // for out stations 
        if($this->session->userdata('role') == 'OS')
        {
            return $query = $this->db->query("UPDATE `students_os` SET `roll_no` = '$roll_no', `operator_id` = '$operator_id', `semester` = '1' WHERE student_id = $student_id "); 
        }else{
            return $query = $this->db->query("UPDATE `students` SET `roll_no` = '$roll_no', `operator_id` = '$operator_id', `semester` = '1' WHERE student_id = $student_id "); 
        }
    }
    ...
    function getPrintChallans($campaign_id, $campus_id, $program_id,$session_id, $start_date, $end_date)
    {
       ...
       if($this->session->userdata('role') == 'OS'){      
                        $query = $this->db->query("SELECT installments.due_date, installments.payable, students.roll_no, students.student_id, forms.form_no, forms.student_name,forms.campaign_id,
                            campaign.campaign_name, campus.campus_name, programs.program_name, banks.bank_name, banks.bank_address, sessions.session,
    ...
    function changeDueDate($campus_id, $program_id, $start_date, $end_date)
    {      
        if($campus_id == 1 || $campus_id == 3)
        {
            $campus         =   "AND (forms.campus_id = 1 OR forms.campus_id = 3)"  ;
    ...
    function updateStatus($student_id,$status)
    {
       ...
       if($this->session->userdata('role') == 'OS'){
           ...
           $id              =    $this->db->insert_id();
           if($id){
            return $query2   = $this->db->query("update students_os set status = '$status' where student_id = $student_id ");
           }else{
            return false;
           }
           ...
           $id              =    $this->db->insert_id();
           if($id){
            return  $query2 = $this->db->query("update students set status = '$status' where student_id = $student_id ");
           }else{
            return false;
           }
   ...
   function challanStatusUnpaid($challan_id)
    ...
    function promoteStudents($program_id,$pre_session_id){
        
         if($this->session->userdata('role') == 'OS')
        {
            $campus_id  =   $this->session->userdata('campus_id');
            $query      =   $this->db->query("SELECT students.*,sfp.session_fee,sfp.misc_fee
                                                FROM students_os AS students
