     ...
     // **************START           Entry Test Module         ************** \\
        ...
        // check duplication of test name
            ...
            {                
                ...
                $query  = $this->db->query("SELECT count(*)AS students FROM forms where program_id = $program_id and campaign_id = $campaign_id and campus_id = $campus_id and form_submit_date >= '$start_date' ");
                $result = $query->result_array();
            ...
            {
                $campaign_id    =   $this->session->userdata('campaign_id');
                $query  = $this->db->query("select form_id  from forms where program_id = $programid and form_id >= $start_form_id and campaign_id = $campaign_id ORDER BY form_id asc limit $students");               
		...
		return $query->result_array();
            ...
            function getStartIdForm($programid){
                $campaign_id    =   $this->session->userdata('campaign_id');
                $query      =   $this->db->query("SELECT * FROM forms WHERE program_id = $programid AND campaign_id = $campaign_id ORDER BY form_id ASC limit 1");
                $result     =   $query->row();
