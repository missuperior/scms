...
class Admin_model extends CI_Model {
    ...
    function getSession($session_id){
        $query  =   $this->db->query("select session from sessions where session_id = $session_id ");        
        $result =   $query->row();
        return $result->session;
    }
    function getPrograms($program_id){
        //echo "select program_id , program_name from programs WHERE program_id = $program_id ";echo '<br/>';die;
        
        $query  =   $this->db->query("select program_id , program_name FROM  programs WHERE program_id = $program_id ");        
    ...
    function getCurrentbatches($campaign_id)
    {      
        $query = $this->db->get_where('batch', array('campaign_id' => $campaign_id));	
        return $query->result_array();
    }
...
//    }
    ...
    function checkProgramFee($date,$program_id,$campus_id,$campaign_id)
    {
    ...
    function getAllprogramsFee($campaign_id)
    {
        $this->db->select('program_fees.*,campaign.campaign_name');
   ...
   //   ***** Start function for product Module *****   //    
    ...
    //--------------Added By Zohaib News ENds----------\\   
      ...
      //   ***** Start function for User Module *****   //
    ...
    {            
        $this->db->select('gsl.sub_login_id,gsl.sub_login,gsl.account_role_id,gsl.role');
        $this->db->select('campus.campus_name');
        $this->db->from('gen_sub_logins gsl');
        $this->db->join('campus', 'campus.campus_id = gsl.campus_id');
        $this->db->order_by("gsl.sub_login_id", "desc");         
        $query = $this->db->get();
    ...
    function updateUserPassword($id,$password)
    {
        ...
        $query = $this->db->update('gen_sub_logins', array('sub_password'=>$password)); 
