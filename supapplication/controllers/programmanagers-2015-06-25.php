...
//            $cur_session_id  = $current_session[0]['session_id']; 
 ...
 /*****************Generte progrma manager login ends  **************************/ 
    ...
    public function add_course_form() {
        ...
        //echo '<pre>';var_dump($allcourses);exit;
        
        $html .= '<select name="pre_reqs[]"><option value="">Program Name -- Course Name -- Course Code</option>'; 
            foreach( $allcourses as $pp){
                $html .='<option value='.$pp["course_id"].'>'.$pp["program_name"].' -- '.$pp["course_name"].' -- '.$pp["course_code"].'</option>';
            }
        $html .= '</select>'; 
