...
//        echo $this->db->last_query();die;
    ...
    function classWiseFinalReport_cr($program_id,$section,$batch_id,$session_id){
        ...
        $query      =   $this->db->query("SELECT students.enrolled_session_id,students.`student_id`,students.roll_no,forms.student_name,final_result.`program_id`,final_result.`course_id` AS final_course,
                                        ...
                                        mid_result.`course_id`  AS mid_course,
                                        final_result.semester, courses.`course_name`,courses.credit_hours,programs.`program_name`,campaign.`campaign_name`,campus.`campus_name` ,batch.*
    ...
    function calculateGpa($obtained1,$obtained2,$exam_type){
        
        if($exam_type == 'Mid'){
            $total = $obtained1 /$obtained2 * 100;
        }else{
            $total  =   $obtained1+$obtained2;
        }
    ...
    function getLastGpa($student_id,$gpa){

        $query  =   $this->db->query("select gpa from std_sem_gpa where student_id = $student_id");
...
//        echo $gpa;die;
    ...
    function getSessions($student_id)
      ...
      //single student rest
    public function mid_result($student_id , $session_id,$course_id  ){
      ...
      //single student rest
    public function mid_result_pass($student_id ,$course_id  ){
    ...
    public function final_result($student_id , $session_id,$course_id  ){
      ...
      //single student rest
    public function final_result_pass($student_id ,$course_id  ){
    ...
    public function getLabMarksStructure($program_id , $batch_id,$course_id  ,$session_id){
        $data       = $this->db->query("SELECT  course_id FROM courses WHERE parent_course_id = $course_id AND batch_id = $batch_id" );
        $result     = $data->result_array();
        $course_idl = $result[0]['course_id'];
        
        if(!empty($course_idl)){
            $data = $this->db->query("SELECT  final_value_1 FROM final_course_structure WHERE program_id = $program_id AND course_id = $course_idl AND session_id = $session_id and batch_id = $batch_id" );
            $result = $data->row();
...
//         echo $this->db->last_query();die;
    ...
    function SaveStdgpa($student_id,$gpa,$semester){
        $query = $this->db->insert('std_sem_gpa', array('semester'=>$semester,'gpa'=>$gpa, 'student_id'=>$student_id)); 		
        return $this->db->insert_id();
    }
    
    function UpdateStdgpa($student_id,$gpa,$semester){
        $this->db->where(array('student_id'=> $student_id,'semester'=>$semester));
        $this->db->update('std_sem_gpa', array('gpa'=>$gpa)); 
        return $this->db->affected_rows();  
    }
    ...
    function getFailSubjects($student_id){
    ...
    function getFailSubjectsCR($student_id,$batch_id){
...
//      echo '<pre>'; print_r($array);die;
    ...
    function getTopperStudents($campaign_id,$program_id,$semester){
        $query      =   $this->db->query("select student_id from final_result
                                            where campaign_id = $campaign_id and program_id = $program_id and semester = $semester
                                            GROUP BY student_id
