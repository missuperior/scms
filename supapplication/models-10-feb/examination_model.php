...
//                                        ORDER BY students.roll_no ASC");
    ...
    function getSessionsWithOutSummer($student_id)
      {
          $query    = $this->db->query("SELECT m.session_id, s.session
                                            FROM
                                            mid_result AS m
                                            INNER JOIN sessions AS s ON s.`session_id` = m.`session_id`
                                            WHERE 
                                            m.`student_id` = $student_id  AND
                                            s.`session` NOT LIKE '%Summer%'
      ...
      {
          $this->db->select('sessions.session, courses.course_name,courses.course_code,mid.student_id, courses.course_id ,courses.credit_hours');
          $this->db->from('mid_result mid');
    ...
    public function mid_result($student_id , $session_id,$course_id,$batch_id){
      ...
      //single student rest
    public function mid_result_pass($student_id ,$course_code,$batch_id){
        $data1 = $this->db->query("SELECT (m.mid_value_1+ m.mid_value_2 + m.mid_value_3) AS midd FROM mid_result AS m 
                                            INNER JOIN courses ON courses.course_id = m.course_id
                                            WHERE 
                                            m.student_id = $student_id AND
        ...
        $data2 = $this->db->query("SELECT m.`session_id` AS session_id 
                                    FROM mid_result AS m 
                                    INNER JOIN courses ON courses.course_id = m.course_id 
                                    WHERE 
                                    m.student_id = 3407 AND
                                    courses.course_code = 'CS103' AND 
                                    m.batch_id = 49 
                                    ORDER BY 
                                    session_id 
                                    LIMIT 1
        ...
        $res         =  array('midd'=>$result1->midd, 'session_id'=>$result2->session_id);
    ...
    public function final_result($student_id , $session_id,$course_id,$batch_id  ){
      ...
      //single student rest
    public function final_result_pass($student_id ,$course_code,$batch_id){
        $data = $this->db->query("SELECT (final_value_1+ final_value_2 + final_value_3 + final_value_4 + final_value_5 + final_value_6 + final_value_7) AS final, 
                                    courses.credit_hours,courses.course_id 
                                    FROM 
                                    final_result 
                                    INNER JOIN courses ON courses.course_id = final_result.course_id 
                                    WHERE 
...
//        }
   ...
   function findCourseInPreviousSession($student_id,$session_id,$course_code){
       
       $query   =   $this->db->query("SELECT COUNT(*) AS total
                                        FROM
                                        mid_result AS m
                                        INNER JOIN courses AS c ON c.`course_id` = m.`course_id`
                                        WHERE 
                                        m.`student_id` = $student_id  AND
